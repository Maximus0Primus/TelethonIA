...
- [ ] **outcome_tracker Phase C throughput** — Phase C limité à ~378 rows/cycle (time budget 20 min, plafond DexPaprika rate limit 0.45 req/s). 122 rows/cycle restent en queue. Soit baisser `KCO_BATCH_LIMIT` 500 → 400 (cleaner mais same throughput), soit augmenter `KCO_TIME_BUDGET` 20→30 min (mais cron 2h, déjà serré). En fait : pas un vrai bug, fill rate global 60.3% est correct pour memecoins SOL (40% des fresh-mints rugent sans laisser de trace OHLCV). Le script v14e.56b force le marquage `dead_no_ohlcv` après 72h pour le backlog. Pas urgent.
- [x] **outcome_tracker Phase A duplicate key spam** — ✅ DONE v14e.56c. Fix dans `outcome_tracker.py:2034-2068` : Phase A pre-filter par `(kol_group, token_address)` en plus du `mention_id`. Stoppait 9+ ERROR par cron run (toutes les 2h) sur la constraint `uq_kco_kol_token`. Cause : nouvelle mention sur un (kol, token) déjà existant passait le filter mention_id mais cassait l'unique constraint à l'insert.
