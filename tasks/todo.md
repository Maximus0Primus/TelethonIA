...
- [ ] **BSR_AB ⚠️ EXPECTED NEGATIVE — verdict 3j (~Mai 02)** — v14e.43 deployed 10 BSR shadow strats (`_BSR52` SOL + `_BSR55` ETH).
  **Re-vérification 14d post-deploy donne un finding NÉGATIF** : sur 7/7 top SOL strats le filtre `rt_buy_sell_ratio >= 0.52` perd $2-4/d à $20/trade (BE25 $16→$14, FAST $16→$12, etc).
  Cause : reverse-engineer optimisait `WR × sqrt(N)` pas $/d. Les trades BSR<0.52 ont WR bas mais avg pnl haut (+3.5%) — fat-tail moonshots. Le filter sacrifie les pumps occasionnels.
  Confidence révisée : BSR améliore WR ~85% certain ; BSR améliore $/d ~10% certain.
  **Action 3j** : laisse les shadows tourner pour confirmation in-vivo. Si paired-test confirme delta $/d négatif → kill les 10 BSR strats. **NE JAMAIS direct-apply au live**.
