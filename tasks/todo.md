...
- [ ] **BSR_AB ⏳ verdict pending** — v14e.43 deployed: 10 BSR shadow strats (5 SOL `_BSR52` + 5 ETH `_BSR55`) clones des top winners avec gate `rt_buy_sell_ratio >= 0.52/0.55`. Pure shadow A/B, paired-test vs base à N≥30 (~7-14j). NE PAS direct-apply tant que verdict pas confirmé live (règle CLAUDE.md). Liste : `BE25_TP80_SL30_BSR52`, `FAST_TP50_SL30_BSR52`, `FAST_TP50_SL30_S40_BSR52`, `BE15_LOCK5_TP50_SL30_BSR52`, `SCALP_TP15_NOSL_S35_BSR52` (SOL); `ETH_TP80_SL40_T2H_BSR55`, `ETH_BE25_LOCK10_TP80_SL40_T2H_BSR55`, `ETH_BE15_LOCK5_TP80_SL30_BSR55`, `ETH_BE15_LOCK10_TP80_SL30_BSR55`, `ETH_BE25_LOCK15_TP100_SL40_T2H_BSR55` (ETH).
- [ ] **SCORE_V2_AB ⏳ data collection** — v14e.43 deployed: `_rt_compute_score_v2()` calculé en parallèle de `rt_score` à chaque RT detect, persisted en colonne `paper_trades.rt_score_v2`. Formula: `30*kol_win_rate + 5*min(bsr,5) + 8*log10(volume+1) + 8*log10(mcap+1)`, clip 0-100. PAS appliqué au filtrage (data only). Verdict après N≥500 closed trades (~7-14j) : si AUC vs WR meilleur que rt_score actuel, proposer swap ou nouveau filter family `SCOREv2_*`.
- [ ] **MEGA_BSR_FILTER ⏳ post-prod** — `_MEGA_EXT_FILTERS` ajoute `BSR52`, `BSR55`, `NOZEROLIQ_BSR52`, `NOZEROLIQ_BSR55` (sim.py:4231) + filter logic `_mega_apply_filter`. Le prochain run de sweep (cron 02:00 UTC SOL ou workflow_dispatch) va ranker ces nouveaux filters dans le grid `mega_sweep_runs`.
